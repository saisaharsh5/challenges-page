...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'static_content' 
...
-- Drop existing policies for static_content if they exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'static_content' 
    AND policyname = 'Allow authenticated users full access to static_content'
  ) THEN
    DROP POLICY "Allow authenticated users full access to static_content" ON static_content;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'static_content' 
    AND policyname = 'Allow public read access on static_content'
  ) THEN
    DROP POLICY "Allow public read access on static_content" ON static_content;
  END IF;
END $$;
...
-- Enable RLS for tryhackme_rooms
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'tryhackme_rooms' 
    AND rowsecurity = true
  ) THEN
...
-- Drop existing policies for tryhackme_rooms if they exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'tryhackme_rooms' 
    AND policyname = 'Allow authenticated users full access to tryhackme_rooms'
  ) THEN
    DROP POLICY "Allow authenticated users full access to tryhackme_rooms" ON tryhackme_rooms;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'tryhackme_rooms' 
    AND policyname = 'Allow public read access on tryhackme_rooms'
  ) THEN
    DROP POLICY "Allow public read access on tryhackme_rooms" ON tryhackme_rooms;
  END IF;
END $$;

-- Create policies for tryhackme_rooms
...
-- Enable RLS for hackthebox_machines
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'hackthebox_machines' 
    AND rowsecurity = true
  ) THEN
...
-- Drop existing policies for hackthebox_machines if they exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'hackthebox_machines' 
    AND policyname = 'Allow authenticated users full access to hackthebox_machines'
  ) THEN
    DROP POLICY "Allow authenticated users full access to hackthebox_machines" ON hackthebox_machines;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'hackthebox_machines' 
    AND policyname = 'Allow public read access on hackthebox_machines'
  ) THEN
    DROP POLICY "Allow public read access on hackthebox_machines" ON hackthebox_machines;
  END IF;
END $$;

-- Create policies for hackthebox_machines
...
-- Enable RLS for ctf_challenges
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'ctf_challenges' 
    AND rowsecurity = true
  ) THEN
...
-- Drop existing policies for ctf_challenges if they exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ctf_challenges' 
    AND policyname = 'Allow authenticated users full access to ctf_challenges'
  ) THEN
    DROP POLICY "Allow authenticated users full access to ctf_challenges" ON ctf_challenges;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ctf_challenges' 
    AND policyname = 'Allow public read access on ctf_challenges'
  ) THEN
    DROP POLICY "Allow public read access on ctf_challenges" ON ctf_challenges;
  END IF;
END $$;

-- Create policies for ctf_challenges
...
BEGIN
  IF EXISTS (
    ...
    DROP TRIGGER update_static_content_updated_at ON static_content;
  END IF;
END $$;
